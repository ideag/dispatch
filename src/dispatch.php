 ...
 */
class D {
  static function error($code, $callback = null) {
   ...
   * to D::config('source', 'inifile.ini') will aggregate the contents of the ini
   * file into D::config().
   *
  ...
  static function config($key = null, $value = null) {
    ...
    if ($key === 'source') {
      if (!file_exists($value)) {
        trigger_error(
          "File passed to D::config('source') not found",
          E_USER_ERROR
        );
      }
   ...
   * Calling D::flash('key') will return the message and remove
   * the message making it unavailable in the following request.
   * Calling D::flash('key', 'message', true) will store that message
   ...
   */
  static function flash($key, $msg = null, $now = false) {
    ...
    if ($msg == null) {
      ...
      if (isset($store[$key])) {
        $cache[$key] = $store[$key];
        unset($store[$key]);
        D::cookie($token, json_encode($store));
      }
   ...
   */
  static function html($str, $flags = -1, $enc = 'UTF-8', $denc = true) {
   ...
   */
  static function params($name = null, $default = null) {
    ...
    if (!$source) {
      ...
      if (get_magic_quotes_gpc()) {
        if (!function_exists('utility_source')) {
          function utility_source(&$v) { $v = stripslashes($v); }
        }
    ...
    // used by D::on() for merging in route symbols.
  ...
  static function session($name, $value = null) {
    ...
    if ($session_active === false) {
      ...
      if (($current = ini_get('session.use_trans_sid')) === false) {
        trigger_error(
          'Call to D::session() requires that sessions be enabled in PHP',
          E_USER_ERROR
        );
      }
   ...
   */
  static function cookie($name, $value = null, $expire = 31536000, $path = '/') {
   ...
   */
  static function request_headers($key = null) {
  ...
  /**
   * Convenience static function for reading in the request body. JSON
   ...
   */
  static function request_body($load = true) {
   ...
   * @return void
   */
  static function send($path, $filename, $sec_expires = 0) {
   ...
   */
  static function scope($name = null, $value = null) {
   ...
   * @return void
   */
  static function redirect($path, $code = 302, $condition = true) {
    if (!$condition)
      return;
  ...
  /**
   * Convenience static function for storing/fetching content to be
   * plugged into the layout within D::render().
   *
   * @param string $value optional, value to use as content.
   *
   * @return string content
   */
  static function content($value = null) {
    return D::scope('$content$', $value);
  }
   ...
   */
  static function template($view, $locals = null) {

    if (($view_root = D::config('dispatch.views')) == null)
      trigger_error("D::config('dispatch.views') is not set.", E_USER_ERROR);
   ...
   * @return string content of the partial.
   */
  static function partial($view, $locals = null) {
    $path = basename($view);
    $view = preg_replace('/'.$path.'$/', "_{$path}", $view);
    return D::template($view, $locals);
  }
   ...
   */
  static function render($view, $locals = array(), $layout = null) {

    // load the template and plug it into D::content()
    $content = D::template($view, $locals);
    D::content(trim($content));

    // layout = null means use the default
    if ($layout == null) {
      $layout = D::config('dispatch.layout');
      $layout = ($layout === null) ? 'layout' : $layout;
    }
    // if we're to use a layout
    if ($layout !== false) {
      // load the layout template, with D::content() already populated
      return (print D::template($layout, $locals));
    }
   ...
   *
   * @return callable handler static function
   */
  static function inline($file, $locals = array(), $layout = 'layout') {
    $locals = is_callable($locals) ? $locals() : $locals;
    if (!function_exists('utility_result')) {
      function utility_result($file, $locals, $layout) {
        return D::render($file, $locals, $layout);
      }
    }
    $result = utility_result($file, $locals, $layout); 
    return $result;
  }
   ...
   * @return void
   */
  static function json($obj, $func = null) {
    D::nocache();
    if (!$func) {
   ...
   * @return void
   */
  static function filter($symbol, $callback = null) {
   ...
   * parameter can be accessed using the param() static function.
   *
   ...
   */
  static function bind($symbol, $callback = null) {
   ...
   * @return void
   */
  static function after($method_or_cb = null, $path = null) {
   ...
   * @param callable $cb required. static function containing route calls
   *
   * @return void
   */
  static function prefix($name = null, $cb = null) {
    ...
    if ($nargs < 2)
      trigger_error("Invalid call to D::prefix()", E_USER_ERROR);
   ...
   * @return void
   */
  static function on($method, $path, $callback = null) {
    ...
    if (is_callable($callback)) {
      ...
      if (!function_exists('utility_regexp')) {
        function utility_regexp($matches) {
    ...
    if (!function_exists('utility_finder')) {
      function utility_finder ($routes, $path) {
        $found = false;
    ...
    if (isset($routes[$method]))
      list($regexp, $callback, $values) = utility_finder($routes[$method], $path);
    ...
    if ($regexp) {
      ...
      call_user_func_array($callback, array_values(D::bind($values)));
      D::after($method, $path);
      $buff = ob_get_clean();
  ...
  static function dispatch() {
    ...
    if ($method == 'POST') {
      if (isset($_SERVER['HTTP_X_HTTP_METHOD_OVERRIDE']))
        ...
        $method = D::params('_method') ? D::params('_method') : $method;
    }
    ...
    if ($stub = D::config('dispatch.router')) {
      $stub = D::config('dispatch.router');
